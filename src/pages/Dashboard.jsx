...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // --- 1. TABEL PENGUMUMAN DENGAN INFO JAM & LOKASI ---
      ...
      // --- 2. TUGAS PRIBADI (URUTAN: Assigned > Submitted > Completed) ---
      ...
      // --- 3. GRAFIK PERSONEL (KABID & KASUBID) ---
  ...
  const formatTime = (timeStr) => {
    if (!timeStr) return '--:--';
    return timeStr.substring(0, 5); // Ambil jam:menit
  };

  if (loading) return <div className="h-screen flex items-center justify-center bg-[#020617] text-blue-400 font-mono text-[10px] tracking-[0.3em]">SYNCHRONIZING SECURE SYSTEMS...</div>;

  return (
    <div className="p-4 space-y-6 bg-[#020617] min-h-screen text-white font-sans text-left overflow-x-hidden">
      ...
      {/* --- TABEL PENGUMUMAN (OPERATIONAL LOG) - DIPERBARUI --- */}
      <div className="bg-[#0c1225] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-3 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
          ...
          <span className="text-[8px] font-mono text-white-500 font-black">Kegiatan Terbaru</span>
        </div>
              ...
              {recentActivities.map((act) => (
                <tr key={act.id} className="hover:bg-white/[0.01] transition-colors group">
                  <td className="p-4 w-12">
                    <div className="w-9 h-9 rounded-full border border-blue-500/30 overflow-hidden bg-slate-800 shadow-inner">
                      {act.profiles?.avatar_url ? (
                        <img src={act.profiles.avatar_url} className="w-full h-full object-cover" alt="User" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-[11px] font-black text-blue-400">{act.profiles?.full_name?.charAt(0)}</div>
                      )}
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="text-[12px] font-black text-white uppercase group-hover:text-blue-400 transition-colors">{act.judul}</div>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex flex-col items-end gap-1">
                      <div className="flex items-center gap-1.5 text-[10px] font-black text-white bg-blue-900/40 px-2 py-0.5 rounded border border-blue-500/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div key={idx} className="bg-white/[0.03] border border-white/10 p-4 rounded-xl flex items-center gap-4 hover:bg-white/[0.05] transition-all">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1.5 text-white">
                      ...
                      <span className="text-[9px] font-mono text-yellow-400 font-bold tracking-widest">{person.total} Penugasan</span>
                    </div>
        ...
        {/* PANEL TUGAS (DENGAN URUTAN TERBAIK) */}
                ...
                return (
                  <div key={idx} className={`bg-white/[0.03] border-l-4 ${style.border} p-3 rounded-r-lg`}>
                    ...
                    <div className="bg-black/50 p-2 rounded border border-white/5 mb-3">
                      <p className="text-[10px] text-white font-medium italic leading-relaxed line-clamp-3">
                        "{item.tasks?.narasi}"
                      </p>
                    </div>
                    <div className="flex items-center justify-between pt-1">
                      <div className="flex items-center gap-2">
                         <div className="w-5 h-5 rounded-full bg-blue-900 flex items-center justify-center text-[7px] font-black">P</div>
